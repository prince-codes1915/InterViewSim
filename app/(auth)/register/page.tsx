...
import { Mic, Mail, Lock, User, UserPlus, AlertCircle, ArrowRight } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setDemoUser(email || "candidate@example.com", name || "Candidate");
      router.push("/dashboard");
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-[85vh] flex items-center justify-center px-4 py-12 relative">
      <div className="absolute inset-0 bg-gradient-to-br from-purple-900/10 via-indigo-900/10 to-slate-950 pointer-events-none" />

      <Card className="w-full max-w-md relative z-10 border-slate-800 bg-slate-900/90 shadow-2xl backdrop-blur-2xl">
        <CardHeader className="text-center space-y-2">
          <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-tr from-indigo-600 to-purple-600 shadow-lg shadow-indigo-500/30">
            <Mic className="h-6 w-6 text-white" />
          </div>
          <CardTitle className="text-2xl font-bold">Create Account</CardTitle>
          <CardDescription>Start practicing voice interviews with Gemini 2.5 AI</CardDescription>
        </CardHeader>
        ...
        <CardContent>
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <Button type="submit" variant="primary" size="lg" className="w-full mt-2" disabled={loading}>
              ...
              {loading ? "Creating Account..." : "Create Free Candidate Account"}
            </Button>
          </form>
          ...
          <Button variant="outline" size="md" onClick={handleDemoSignIn} className="w-full">
            Explore Demo Account Immediately <ArrowRight className="h-4 w-4 ml-2" />
          </Button>
        </CardContent>

        <CardFooter className="justify-center text-xs text-slate-400">
          Already have an account?{" "}
          <Link href="/login" className="text-indigo-400 font-semibold hover:underline ml-1">
            Sign In
          </Link>
        </CardFooter>
      </Card>
